...
class SpaceErrors:
    ...
    OPERATOR_REQUIRES_DUAL_VERIFICATION = Error(message=_('Operators must have verified email and phone contacts'), code=Code.BadRequest)
    OPERATOR_TARGET_INVALID = Error(message=_('The operator must be an active member of this space'), code=Code.BadRequest)
    ...
    SQUARE_MODES_REQUIRED = Error(message=_('Free posting and submissions cannot both be disabled while square is enabled'), code=Code.BadRequest)
    ...
    SQUARE_FREE_POST_DISABLED = Error(message=_('Free posting is disabled in this space'), code=Code.Forbidden)
