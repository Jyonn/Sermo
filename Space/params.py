...
class SpaceParams(metaclass=Params):
    ...
    space_group_enabled = Validator('space_group_enabled').to(int).null().default(None).bool(
        lambda x: x is None or x in (0, 1), message=_('space_group_enabled should be 0 or 1'))
    submission_enabled = Validator('submission_enabled').to(int).null().default(0).bool(
        lambda x: x in (0, 1), message=_('submission_enabled should be 0 or 1'))
    square_free_post_enabled = Validator('square_free_post_enabled').to(int).null().default(None).bool(
        lambda x: x is None or x in (0, 1), message=_('square_free_post_enabled should be 0 or 1'))
