...
EVENT_RULES = {
    rule.key: rule for rule in (
        ...
        GrowthEventRule('explore:file', 'explore', '首次发送文件', 15),
        GrowthEventRule('explore:link', 'explore', '首次发送链接卡片', 10),
        GrowthEventRule('explore:sticker_collect', 'explore', '首次收藏表情包', 10),
        GrowthEventRule('explore:sticker_create', 'explore', '首次制作表情包', 20),
        GrowthEventRule('explore:sticker_send', 'explore', '首次发送表情包', 10),
        ...
        GrowthEventRule('explore:square_video', 'explore', '首次发表视频发言', 25),
        GrowthEventRule('explore:square_friends', 'explore', '首次发表好友圈发言', 10),
        GrowthEventRule('explore:square_comment', 'explore', '首次评论发言', 15),
        GrowthEventRule('explore:square_reply', 'explore', '首次回复评论', 15),
        GrowthEventRule('explore:square_like', 'explore', '首次点赞发言', 5),
        GrowthEventRule('explore:square_comment_like', 'explore', '首次点赞评论', 5),
...
PERSONALIZATION_LEVELS = {
    'chat_bubble_style': {
        'default': 1, 'comic': 2, 'typewriter': 4,
...
LEVEL_REWARDS = {
    ...
    4: [_reward('capability.group', 4, 'capability', '创建群聊并邀请好友', 'rare', capability_key='create_group'), _reward('capability.avatar', 4, 'capability', '自定义头像', 'rare', capability_key='custom_avatar'), _reward('bubble.typewriter', 4, 'bubble', '打字机气泡', 'uncommon', 'typewriter'), _reward('frame.camera', 4, 'frame', '相机头像框', 'uncommon', 'camera')],
    5: [_reward('capability.video', 5, 'capability', '发送视频', 'rare', capability_key='send_video'), _reward('capability.group_name', 5, 'capability', '修改群名称', 'uncommon', capability_key='rename_group'), _reward('capability.nickname_365', 5, 'capability', '昵称每 365 天可改', 'uncommon', capability_key='rename_nickname'), _reward('background.comic', 5, 'background', '漫画背景', 'uncommon', 'comic'), _reward('frame.soundwave', 5, 'frame', '声波头像框', 'rare', 'soundwave')],
    6: [_reward('capability.welcome', 6, 'capability', '自定义欢迎语与广场招呼', 'rare', capability_key='welcome_message'), _reward('capability.sticker', 6, 'capability', '制作个人表情包', 'rare', capability_key='create_sticker'), _reward('background.zen', 6, 'background', '枯山水背景', 'rare', 'zen'), _reward('bubble.zen', 6, 'bubble', '禅意气泡', 'uncommon', 'zen'), _reward('frame.butterfly', 6, 'frame', '蝴蝶头像框', 'legendary', 'butterfly')],
    7: [_reward('capability.online', 7, 'capability', '好友上线提醒', 'rare', capability_key='online_reminder'), _reward('bubble.newspaper', 7, 'bubble', '报纸气泡', 'uncommon', 'newspaper'), _reward('frame.aurora', 7, 'frame', '极光头像框', 'epic', 'aurora')],
    8: [_reward('capability.audio_download', 8, 'capability', '下载语音', 'uncommon', capability_key='download_audio'), _reward('capability.nickname_30', 8, 'capability', '昵称每 30 天可改', 'rare', capability_key='rename_nickname'), _reward('capability.custom_background', 8, 'capability', '上传自定义聊天背景', 'epic', capability_key='custom_chat_background'), _reward('background.dragon', 8, 'background', '游龙背景', 'uncommon', 'dragon'), _reward('background.bauhaus', 8, 'background', '包豪斯背景', 'uncommon', 'bauhaus'), _reward('frame.moon', 8, 'frame', '月相头像框', 'rare', 'moon')],
    9: [_reward('background.mosaic', 9, 'background', '马赛克背景', 'rare', 'mosaic'), _reward('bubble.hero', 9, 'bubble', '英雄气泡', 'rare', 'hero'), _reward('frame.papercut', 9, 'frame', '剪纸头像框', 'rare', 'papercut')],
    10: [_reward('bubble.bauhaus', 10, 'bubble', '包豪斯气泡', 'rare', 'bauhaus'), _reward('frame.comet', 10, 'frame', '彗星头像框', 'legendary', 'comet')],
    11: [_reward('bubble.receipt', 11, 'bubble', '小票气泡', 'rare', 'receipt'), _reward('frame.snowfall', 11, 'frame', '落雪头像框', 'epic', 'snowfall')],
    12: [_reward('capability.nickname_7', 12, 'capability', '昵称每 7 天可改', 'epic', capability_key='rename_nickname'), _reward('bubble.dragon', 12, 'bubble', '游龙气泡', 'epic', 'dragon'), _reward('frame.portal', 12, 'frame', '传送门头像框', 'epic', 'portal'), _reward('profile.card_theme', 12, 'identity', '个人名片主题系统', 'legendary', status='planned')],
    13: [_reward('bubble.mosaic', 13, 'bubble', '马赛克气泡', 'epic', 'mosaic'), _reward('frame.mechanical', 13, 'frame', '机械头像框', 'epic', 'mechanical')],
